...
struct BeforeAfterResults: View {
  ...
  /// 作例ごとに被写体と見どころを喋る。本番で「この花は何だっけ」と詰まらないよう、
  /// 被写体の名前を原稿の先頭に置いておく。`pairs` の並びと対応させること。
  var script: String {
    switch phase {
    case .initial:
      return """
        ここまでの方法で実際に処理してみた写真です。左が元の写真、右が DA3-SMALL で深度を推定して CIBokehBlur をかけたものです。
        ボケだけでなく色味も変わって見えると思いますが、これは自作している ToneCraft というアプリで編集をしています。
        1枚目はマツバギクです。手前の花だけがシャープに残って、奥の花と壁のタイルがなだらかにボケています。
        """
    case .second:
      return """
        2枚目は紫陽花です。奥のバスと建物がしっかりボケて、手前の紫陽花が浮き上がって見えます。
        """
    case .third:
      return """
        3枚目は彼岸花です。細い花びらの輪郭がきれいに残ったまま、奥の木立と一面の彼岸花がボケているのがポイントです。
        """
    case .fourth:
      return """
        4枚目はカフェのプリンとコーヒーです。テーブルの手前と奥がボケて、トレイの上だけにピントが合っています。
        """
    case .fifth:
      return """
        5枚目はベッドの上の猫です。顔にピントが合っていて、ヘッドボードと奥の壁が段階的にボケていきます。
        """
    case .sixth:
      return """
        6枚目はキャットタワーの上の猫です。ふわふわの毛の輪郭が崩れずに、奥のカーテンと部屋がボケています。
        """
    case .seventh:
      return """
        最後はひまわり畑です。冒頭で「どっちが iPhone で撮ったでしょう」と出した写真です。手前の一輪だけがくっきり残って、後ろのひまわりと夕焼けの空がボケて、ミラーレスで撮ったような雰囲気になっています。
        """
    }
  }
