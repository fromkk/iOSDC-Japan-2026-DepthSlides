...
enum RayCanvasDrawing {
  ...
  static func fitTransform(
    minX: CGFloat, maxX: CGFloat, maxAbsHeight: CGFloat, in size: CGSize, margin: CGFloat = 28
  ) -> Transform {
  ...
  static func drawAxis(
    _ context: inout GraphicsContext, minX: CGFloat, maxX: CGFloat, transform: Transform
  ) {
    var path = Path()
    path.move(to: transform.point(CGPoint(x: minX, y: 0)))
    path.addLine(to: transform.point(CGPoint(x: maxX, y: 0)))
    context.stroke(
      path, with: .color(.secondary.opacity(0.3)), style: StrokeStyle(lineWidth: 1, dash: [2, 4]))
  }

  static func drawArrowObject(
    _ context: inout GraphicsContext, base: CGPoint, tip: CGPoint, transform: Transform,
    color: Color = .primary
  ) {
    ...
    let left = CGPoint(
      x: t.x - headLength * cos(angle - headAngle), y: t.y - headLength * sin(angle - headAngle))
    let right = CGPoint(
      x: t.x - headLength * cos(angle + headAngle), y: t.y - headLength * sin(angle + headAngle))
    var head = Path()
    head.move(to: t)
  ...
  static func drawWallWithGap(
    _ context: inout GraphicsContext, x: CGFloat, gapCenter: CGFloat, gapSize: CGFloat,
    halfExtent: CGFloat, transform: Transform
  ) {
  ...
  static func drawLensShape(
    _ context: inout GraphicsContext, x: CGFloat, halfHeight: CGFloat, transform: Transform
  ) {
    ...
    context.stroke(
      axis, with: .color(.cyan.opacity(0.4)), style: StrokeStyle(lineWidth: 1, dash: [2, 3]))
  }

  static func drawScreen(
    _ context: inout GraphicsContext, x: CGFloat, halfExtent: CGFloat, transform: Transform
  ) {
    var path = Path()
  ...
  static func drawMarker(
    _ context: inout GraphicsContext, at point: CGPoint, transform: Transform, color: Color,
    radius: CGFloat = 3
  ) {
