            ...
            VStack(alignment: .leading, spacing: 8) {
                GeometryReader { geometry in
                    Canvas { context, size in
                        ...
                        let maxAbsHeight = max(scene.objectHeight, abs(scene.imageHeight ?? 0), scene.focalLength * 0.3) + 15
                        let transform = RayCanvasDrawing.fitTransform(minX: minX, maxX: maxX, maxAbsHeight: maxAbsHeight, in: size)

                        RayCanvasDrawing.drawAxis(&context, minX: minX, maxX: maxX, transform: transform)
                        RayCanvasDrawing.drawScreen(&context, x: scene.screenX, halfExtent: maxAbsHeight, transform: transform)
                        RayCanvasDrawing.drawLensShape(&context, x: scene.lensX, halfHeight: maxAbsHeight * 0.85, transform: transform)
                        RayCanvasDrawing.drawMarker(&context, at: scene.farFocalPoint, transform: transform, color: .secondary)
                        RayCanvasDrawing.drawMarker(&context, at: scene.nearFocalPoint, transform: transform, color: .secondary)
                        ...
                        RayCanvasDrawing.drawArrowObject(&context, base: CGPoint(x: scene.objectX, y: 0), tip: CGPoint(x: scene.objectX, y: scene.objectHeight), transform: transform)

                        if let di, let hi = scene.imageHeight {
                            RayCanvasDrawing.drawArrowObject(&context, base: CGPoint(x: di, y: 0), tip: CGPoint(x: di, y: hi), transform: transform, color: .orange)
                            RayCanvasDrawing.drawMarker(&context, at: CGPoint(x: di, y: hi), transform: transform, color: .orange)
                        }
                    }
