...
struct PinholeScene: Equatable {
    ...
    func projectedScreenPoint(for objectPoint: CGPoint, apertureY: CGFloat) -> CGPoint {
        Geometry.point(onLineThrough: objectPoint, and: CGPoint(x: barrierX, y: apertureY), atX: screenX)
    }
    ...
    /// point.
    func trace(objectPoint: CGPoint) -> (surviving: LineSegment, screenPoint: CGPoint, blocked: [LineSegment]) {
    ...
    func blurEdgeRays(objectPoint: CGPoint) -> [LineSegment] {
        ...
        return [apertureHeight - gapEdge, apertureHeight + gapEdge].map { edgeY in
            LineSegment(start: objectPoint, end: projectedScreenPoint(for: objectPoint, apertureY: edgeY), style: .active)
        }
    }
}
